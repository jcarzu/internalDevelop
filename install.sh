...
if [[ "${build_release}" == true ]]; then
  rm -rf ${build_dir}/release/rocblas
  rm -rf ${build_dir}/release/rocsolver
else
  rm -rf ${build_dir}/debug/rocblas
  rm -rf ${build_dir}/debug/rocsolver
fi
...
pushd .
  ...
  if [[ "${build_release}" == true ]]; then
    mkdir -p ${build_dir}/release/rocblas/clients
    if [[ "${build_rocsolver}" == true ]]; then
      mkdir -p ${build_dir}/release/rocsolver/clients
    fi
    cmake_common_options="${cmake_common_options} -DCMAKE_BUILD_TYPE=Release"
  else
    mkdir -p ${build_dir}/debug/clients 
    if [[ "${build_rocsolver}" == true ]]; then
      mkdir -p ${build_dir}/debug/rocsolver/clients
    fi
  ...
  cd ${build_dir}/release/rocblas
  CXX=${compiler} ${cmake_executable} ${cmake_common_options} ${tensile_opt} ${cmake_client_options} -DCPACK_SET_DESTDIR=OFF -DCMAKE_INSTALL_PREFIX=to_install -DCPACK_PACKAGING_INSTALL_PREFIX=/opt/rocm ../../../rocblas
  check_exit_code "$?"
  ...
  if [[ "${build_rocsolver}" == true ]]; then
    cd ../rocsolver
    rocblas_DIR=../rocblas/to_install/ CXX=${compiler} ${cmake_executable} ${cmake_common_options} ${tensile_opt} ${rocsolver_opt} ${cmake_client_options} -DCPACK_SET_DESTDIR=OFF -DCMAKE_INSTALL_PREFIX=to_install -DCPACK_PACKAGING_INSTALL_PREFIX=/opt/rocm ../../../rocsolver
    check_exit_code "$?"
    make -j$(nproc) install
    check_exit_code "$?"
  fi
